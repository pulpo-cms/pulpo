# Use singleton class Pulpo::Preferences::Store.instance to access
#
...
module Pulpo::Preferences
  class StoreInstance
    ...
    def exist?(key)
      @cache.exist?(key) ||
        should_persist? && Pulpo::Preference.where(key: key).exists?
    end
    ...
    def get(key)
      ...
      if should_persist?
        ...
        val = if preference = Pulpo::Preference.find_by(key: key)
          # it does exist
                preference.value
              else
    ...
    def persist(cache_key, value)
      ...
      preference = Pulpo::Preference.where(key: cache_key).first_or_initialize
    ...
    def destroy(cache_key)
      ...
      preference = Pulpo::Preference.find_by(key: cache_key)
      preference.destroy if preference
    end

    def should_persist?
      @persistence && Pulpo::Preference.table_exists?
